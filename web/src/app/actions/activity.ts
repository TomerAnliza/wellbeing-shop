...
/**
 * שמירת אימון שהסתיים. הדפדפן שולח רק סוג, זמני התחלה וסיום, ושניות פעילות נטו (בלי השהיות).
 * השרת בודק אותם ומחשב בעצמו משך, מרחק וקלוריות — לא סומכים על מספרים מהדפדפן.
 */
export async function saveActivity(input: {
  ...
  const { calories, distanceKm } = estimate(input.type, minutes);
  ...
  const { error } = await supabase.from("activities").insert({
    ...
    calories,
    is_estimated: true, // המדדים נגזרו מזמן, לא נמדדו
  });
