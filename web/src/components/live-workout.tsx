...
// מסך 3 — אימון חי: שכבה כהה מעל הכול, שעון, שלושה מדדים מוערכים,
// ושלושה כפתורים: השהה/המשך, סיום ושמירה, וביטול עם אישור
export function LiveWorkout({ state, units, onChange }: { state: LiveState; units: Units; onChange: (state: LiveState | null) => void }) {
  ...
  const { calories, distanceKm } = estimate(state.type, seconds / 60);
  ...
  function finish() {
    ...
    startSaving(async () => {
      const result = await saveActivity({
        ...
        endedAt,
        activeSeconds: activeSeconds(state, endedAt),
      });
      if (!result.ok) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[80] flex flex-col bg-[#1a1820] px-6 pt-16 pb-10 text-white lg:pt-24"
    >
      <div className="mx-auto flex w-full max-w-[430px] flex-1 flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-7">
          ...
          <div className="flex gap-8 text-center">
            <Metric value={distanceKm === null ? "—" : convertDistance(distanceKm, units).toFixed(2)} label={distanceUnit(units)} estimated={distanceKm !== null} />
            <Metric value={String(calories)} label="קלוריות" estimated />
            <Metric value={paceLabel(state.type, units)} label="קצב" estimated={distanceKm !== null} />
          </div>
        ...
        {confirmCancel ? (
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-3">
              <button
                onClick={() => update(paused ? resume(state) : pause(state))}
                disabled={saving}
...
function Metric({ value, label, estimated }: { value: string; label: string; estimated?: boolean }) {
  return (
    <div>
